...
class CreateTicketForm extends React.Component {
        ...
        return (
            <div className="create-ticket-form">
                ...
                <Form {...this.getFormProps()}>
                    ...
                    <FormField label={i18n('TITLE')} name="title" validation="TITLE" required field="input"
                               fieldProps={{size: 'large'}}/>
                    <div className="row">
                        {!(this.props.isDefaultDepartmentLocked * 1) || this.props.isStaff ?
                            <FormField className="col-md-5" label={i18n('DEPARTMENT')} name="departmentIndex"
                                       field="select" decorator={DepartmentDropdown} fieldProps={{
                        ...
                        {!this.props.onlyOneSupportedLanguage ?
                            <FormField className="col-md-5" label={i18n('LANGUAGE')} name="language" field="select"
                                       decorator={LanguageSelector} fieldProps={{
                                type: 'supported',
                                size: 'medium'
                            }}/> : null
                        }
    ...
    renderEmailAndName() {
        return (
            <div className="row">
                <FormField className="col-md-6" label={i18n('EMAIL')} name="email" validation="EMAIL" required
                           field="input" fieldProps={{size: 'large'}}/>
                <FormField className="col-md-6" label={i18n('FULL_NAME')} name="name" validation="NAME" required
                           field="input" fieldProps={{size: 'large'}}/>
            </div>
        );
    }
    ...
    renderMessage() {
        switch (this.state.message) {
            ...
            case 'fail':
                return <Message className="create-ticket-form__message"
                                type="error">{i18n('TICKET_SENT_ERROR')}</Message>;
            default:
                return null;
        }
    }
    ...
    getFormProps() {
        return {
            ...
            onChange: this.onChange.bind(this),//form => this.setState({form}),
            errors: this.state.formErrors,
            onValidateErrors: errors => this.setState({formErrors: errors})
        };
    }
    ...
    onSubmit(formState) {
        ...
        if (captcha && !captcha.getValue()) {
            ...
            if (formState.departmentIndex === -1) {
                let errors = _.clone(this.state.formErrors);
                errors.departmentIndex = i18n('MANDATORY_FIELD');
